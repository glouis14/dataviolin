...
//	digitalWrite (LEDpin, HIGH);
	...
	//	SMF.setFilename ("SEQ.MID");
		...
		// new multifile selector
		pgm = checkSelector ();
		switch (pgm) {
			case 1: 	SMF.setFilename ("SEQ1.MID");	break;
			case 2: 	SMF.setFilename ("SEQ2.MID");	break;
			case 3: 	SMF.setFilename ("SEQ3.MID");	break;
			case 4: 	SMF.setFilename ("SEQ4.MID");	break;
			case 5: 	SMF.setFilename ("SEQ5.MID");	break;
			case 6: 	SMF.setFilename ("SEQ6.MID");	break;
			case 7: 	SMF.setFilename ("SEQ7.MID");	break;
			case 8: 	SMF.setFilename ("SEQ8.MID");	break;
			case 9: 	SMF.setFilename ("SEQ9.MID");	break;
			default: 	SMF.setFilename ("SEQ1.MID");	break;
		}
...
void PlayNote (int ID, int velocity) {
	long fretPressureLocal = (MAX_PWM_LINE * fretPressure.c) / 127;
	long bowPressure, motorSpeed;
	
	if ((ID != L_OPEN) && (ID != R_OPEN)) {
	//	linePWM (ID, MAX_PWM_LINE, 10, 0);
	//	linePWM (ID, MAX_PWM_LINE, noteOnFretAttack.c, noteOnFretDelay.c);
		linePWM (ID, fretPressureLocal, noteOnFretAttack.c, noteOnFretDelay.c);
		//			  16 bit         7 bit
	}
	...
	if (PWMboard [ID] == B_LEFT) {
		...
		if (note_playing_flag_L) {
			...
			bowPressure	= (MAX_PWM_LINE * bowPressure_L.c * fretBowCompensation [ID]) / 8001;
			//             16 bit         7 bit             7 bit = 30 bit              127*63 (13 bit)
			bowPressure = (bowPressure * potBow) / 127;
			linePWM (L_BOW, bowPressure, noteOnBowAttack.c, noteOnBowDelay.c);
		
			motorSpeed = (MAX_PWM_LINE * motorSpeed_L.c) / 127;
			motorSpeed = (motorSpeed * potMot) / 127;
			linePWM (L_MOTOR, motorSpeed, noteOnMotorAttack.c, noteOnMotorDelay.c);
		}
		...
		if (note_playing_flag_R) {
			...
			bowPressure	= (MAX_PWM_LINE * bowPressure_R.c * fretBowCompensation [ID]) / 8001;
			//             16 bit         7 bit             7 bit = 30 bit              127*63 (13 bit)
			bowPressure = (bowPressure * potBow) / 127;
			linePWM (R_BOW, bowPressure, noteOnBowAttack.c, noteOnBowDelay.c);
		
			motorSpeed = (MAX_PWM_LINE * motorSpeed_R.c) / 127;
			motorSpeed = (motorSpeed * potMot) / 127;
			linePWM (R_MOTOR, motorSpeed, noteOnMotorAttack.c, noteOnMotorDelay.c);
...
void checkAnalog () {
	...
	if (trl < 100) {
		...
		potBow = map (trl, 100, 511, 0, 127);
		potMot = 0;
	}
	else if (trl < 924) {
		potBow = 127;
		potMot = map (trl, 512, 923, 0, 127);
	}
	else {
		potBow = 127;
		potMot = 127;
...
void OnNoteOn (byte channel, byte note, byte velocity) {
	...
	if (localRandomization) {
		...
		// use one of the trimpots to override the minimum fret pressure
		long override_pressure_min, override_pressure_max;
		
		if (tru < 768) {	// only use lower 3/4 of range. if above we keep the setting as stored in EEPROM / transmitted via MIDI
			if (tru < 256) {
				override_pressure_min = map (tru, 0, 255, 127, 60);				
				override_pressure_max = 127;
			}
			else if (tru < 512) {
				override_pressure_min = 60;
				override_pressure_max = map (tru, 256, 511, 127, 80);				
			}
			else {
				override_pressure_min = 60;
				override_pressure_max = 80;
			}
			fretPressure.c = random (override_pressure_min, override_pressure_max);
		//	digitalWrite (LEDpin, HIGH);
		}
		else {
		//	digitalWrite (LEDpin, LOW);
...
//	DEBUGN_LN (value);
	...
	case 2:
		if ((control >= L_OPEN) && (control <= L_FRET_12)) {
			fretBowCompensation [control] = value;
		}
		if ((control >= R_OPEN) && (control <= R_FRET_12)) {
			fretBowCompensation [control] = value;
		}
		break;
